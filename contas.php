...
try {
    ...
    if ($resultado->num_rows > 0) {
        while ($contas = $resultado->fetch_assoc()) {
            $lista[] = $contas;
        }
    }
} catch (Exception $e) {
                ...
                <div class="container-fluid">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addConta">
                                Adicionar Contas
                            </button>
                            <hr>
                            ...
                            <?php if (!empty($lista)) : ?>
                                <div class="table-container">
                                    <table class="table table-hover table-striped table-bordered table-responsive">
                                        ...
                                        <tbody>
                                            <?php foreach ($lista as $contas) : ?>
                                                <tr>

                                                    <td><?= htmlspecialchars($contas['iban']); ?></td>
                                                    <td><?= htmlspecialchars($contas['banco']); ?></td>
                                                    <td>
                                                        <a href="#" class="btn btn-success edita_conta" data-id="<?= $contas['id']; ?>">Editar</a>
                                                        <button type="button" class="btn btn-danger excluir_conta" data-id="<?= $contas['id']; ?>" data-bs-toggle="modal" data-bs-target="#confirmDeleteModal">Excluir</button>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                        </tbody>
                    ...
                    <div class="modal fade" id="addConta" tabindex="-1" aria-labelledby="addCursoLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="addCursoLabel">Adicionar Contas</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <form action="code_contas.php" method="POST">
                                        <div class="mb-3">
                                        ...
                                        <div class="mb-3">
                                            <label for="duracao" class="form-label">Banco</label>
                                            <input type="text" class="form-control" id="banco" name="banco" required>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                                            <button type="submit" name="salvar_conta" class="btn btn-primary">Salvar Conta</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                    ...
                    <div class="modal fade" id="editConta" tabindex="-1" aria-labelledby="editPropinaLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="editPropinaLabel">Editar Contas</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <form action="editar_conta.php" method="POST">
                                        ...
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                                            <button type="submit" name="atualiza_conta" class="btn btn-primary">Atualizar Conta</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                $('#edit_iban').val(response.data.iban);
                $('#edit_banco').val(response.data.banco);
                $('#edit_usuario_id').val(response.data.usuario_id);
                $('#editConta').modal('show');
            } else {
